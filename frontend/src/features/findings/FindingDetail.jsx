...
export default function FindingDetail() {
  ...
  const load = useCallback(async function load() {
    ...
    try {
      ...
      const [auditPage, intelligencePayload, aiArtifacts] = await Promise.all([
        listAuditLogs({ entity_type: 'Finding', entity_id: findingId }),
  ...
  async function refreshIntelligence() {
    ...
    try {
      ...
      setState((current) => ({ ...current, finding: payload.finding, intelligence: payload.intelligence }));
  ...
  async function generateDraft() {
    ...
    try {
      ...
      setState((current) => ({ ...current, aiArtifacts: [artifact, ...current.aiArtifacts.filter((item) => item.id !== artifact.id)] }));
